...
function handleFormSubmit(event) { // handles article form submit withtout any jquery
  ...
  if (wasformsubmitted.value == "YES" && namecheck) { // if vistor clicked YES button show thank you message
    ...
    xhr.onreadystatechange = function() {
      ...
      document.getElementById('yes_thankyou_message').style.display = 'block';
      return;
    };
  ...
  /*else { // if visitor clicked NO button show follow up message
    ...
    xhr.onreadystatechange = function() {
      document.getElementById('gform').style.display = 'none'; // hide form
      document.getElementById('no_thankyou_message').style.display = 'block';
      return;
    };
    // url encode form data for sending as post data
    var encoded = Object.keys(data).map(function(k) {
